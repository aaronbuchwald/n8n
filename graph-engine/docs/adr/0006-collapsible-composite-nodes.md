...
Status: **DEFERRED** (design pass complete; agreement + review intentionally
deferred, 2026-07 — low ROI for current purposes, needs more thought before we
commit). The full design below is **kept on purpose** so we can pick it up
later without re-deriving it; nothing here is approved or scheduled. · Scope:
`graph-engine/` · Relates to: ADR 0001 (engine core), ADR 0004 (graph ⟷ source
bijection), ADR 0005 (node-declared UI), ADR 0008 (sync), ADR 0009 (entry
points)

> Deferral note: the design pass resolved all six open questions into concrete
> recommendations (C1–C8) with code sketches and a build plan — captured here
> so the work is shovel-ready if/when we return to it. **We are deliberately
> not reviewing or agreeing on these decisions now.** Do not implement; do not
> treat the FOR HUMAN REVIEW items below as pending sign-off — they are parked.
